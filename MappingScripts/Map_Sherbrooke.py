...
# reading data
sensor1 = pd.read_csv('lan3(1)_Top.csv')
sensor2 = pd.read_csv('lan3(2)_Back.csv')
sensor3 = pd.read_csv('lan3(3)_Right.csv')
sensor4 = pd.read_csv('lan3(4)_Front.csv')
sensor5 = pd.read_csv('lan3(5)_Left.csv')
sensor6 = pd.read_csv('lan3(6)_Bottom.csv')
...
def color_producer(msi):
    if msi < 0.1 or msi == 0.1:
        return 'red'
    elif msi > 0.1 and msi < 0.2 or msi == 0.2:
        return 'orange'
    elif msi > 0.2 and msi < 0.3 or msi == 0.3:
        return 'yellow'
    elif msi > 0.3 and msi < 0.4 or msi == 0.4:
        return 'lime'
    elif msi > 0.4 and msi < 0.5 or msi == 0.5:
        return 'green'
    elif msi > 0.5 and msi < 0.6 or msi == 0.6:
        return 'cyan'
    elif msi > 0.6 and msi < 0.7 or msi == 0.7:
        return 'blue'
    elif msi > 0.7 and msi < 0.8 or msi == 0.8:
        return 'navy'
    else:
        return 'purple'
    ...
    for lt, ln, at, MSI, SLI in zip(lat, lon, alt, msi, sli):
        layer.add_child(folium.CircleMarker(location=[lt, ln],
                                            ...
                                            popup=folium.Popup('*Latitude:' + str(lt) + ' *Longitude:' + str(ln) +
                                                               '  *MSI:' + str(MSI) + ' *SLI:' +
                                                               str(SLI)),
                                            fill_color=color_producer(MSI),
                                            color=color_producer(MSI),
                                            fill_opacity=1))
    m.add_child(layer)
...
<div class='legend-title'>MSI value</div>

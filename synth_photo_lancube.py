...
resps_interp_ammar = np.array([np.interp(spectra_ammar[0,:,0],resps[:,0],resps[:,i]) for i in [1,2,3,4]])
...
spectra_resps_ammar = np.array([np.multiply(spectra_ammar[:,:,1],resps_interp_ammar[0]),
						np.multiply(spectra_ammar[:,:,1],resps_interp_ammar[1]),
						np.multiply(spectra_ammar[:,:,1],resps_interp_ammar[2]),
						np.multiply(spectra_ammar[:,:,1],resps_interp_ammar[3])])
...
spectra_rgbc_ammar = np.sum(spectra_resps_ammar,axis=2).T
...
R_ammar,G_ammar,B_ammar,C_ammar=[np.array([spectra_rgbc_ammar[:,i]]).T for i in np.arange(spectra_rgbc_ammar.shape[1])]
...
spectra_int_ammar = np.concatenate((spectra_rgbc_ammar,R_ammar/G_ammar,B_ammar/G_ammar,R_ammar/C_ammar,G_ammar/C_ammar,B_ammar/C_ammar),axis=1)
...
indices_ammar = np.array([CalcIndices(spectrum) for spectrum in spectra_ammar])  
...
plt.figure()
plt.hist(indices_ammar[:,0],100,label='MSI', color='b', alpha=0.5)
plt.hist(indices_ammar[:,1],100,label='SLI', color='k', alpha=0.5)
plt.hist(indices_ammar[:,2],100,label='IPI', color='g', alpha=0.5)
...
z_ammar=np.vstack((indices_ammar[:,0],indices_ammar[:,1],indices_ammar[:,2],np.array(fnames_ammar))).T
z_ammar_all=np.concatenate((spectra_int_ammar,z_ammar),axis=1)
final=np.vstack((z_all,z_jo_all,z_ammar_all))

np.savetxt('computed_indices.csv', final, comments='', header='R,G,B,C,R/G,B/G,R/C,B/C,G/C,MSI,SLI,IPI,fname', delimiter=',', fmt='%s')
np.savetxt('computed_indices_ammar.csv', z_ammar_all, comments='', header='R,G,B,C,R/G,B/G,R/C,B/C,G/C,MSI,SLI,IPI,fname', delimiter=',', fmt='%s')
